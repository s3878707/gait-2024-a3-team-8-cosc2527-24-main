...
public class Player : MonoBehaviour
{
    ...
    [SerializeField]
    public GameObject _invincibilityAnim;

    [SerializeField]
    public GameObject _boosterAnim;

    [SerializeField]
    public GameObject _laserPrefab;

    [SerializeField]
    GameObject _spreadShotPrefab;

    [SerializeField]
    public GameObject _shieldGameObject;

    [SerializeField]
    public GameObject _centerGameObject; // The center of the ship from which shots fire out
    ...
    [SerializeField]
    private ControlStyle controlStyle = ControlStyle.Version1;
    ...
    private void FixedUpdate()
    {
        if (_gameManager.isPaused == false)
        {
            ...
            // Top left corner
            if (position.y > topBorder - threshold && position.x < leftBorder + 0.5f)
            {
                GetComponent<PlayerAgent>().HandleCloseToBorder();
            }
            // Top right corner
            else if (position.y > topBorder - threshold && position.x > rightBorder - 0.5f)
            {
                GetComponent<PlayerAgent>().HandleCloseToBorder();
            }
            // Bottom left corner
            else if (position.y < bottomBorder + threshold && position.x < leftBorder + 0.5f)
            {
                GetComponent<PlayerAgent>().HandleCloseToBorder();
            }
            // Bottom right corner
            else if (position.y < bottomBorder + threshold && position.x > rightBorder - 0.5f)
            {
                GetComponent<PlayerAgent>().HandleCloseToBorder();
            }
    ...
    public void Movement_V1(int rotate, int strafe)
    {
        ...
        // GetComponent<PlayerAgent>().MoveAwayFromBorder();
        // GetComponent<PlayerAgent>().IsTooCloseToBoss();
    }
    ...
    public void Shoot()
    {
        if (Time.time > _fireTime)
        {
            ...
            GameObject clone = (GameObject)Instantiate(
                _laserPrefab,
                new Vector3(
                    shipCenter.transform.position.x + laserStartPos.x,
                    shipCenter.transform.position.y + laserStartPos.y,
                    0
                ),
                Quaternion.Euler(0, 0, laserAng)
            );
    ...
    private void spreadLaser(float angle)
    {
        ...
        laserclone = (GameObject)Instantiate(
            _laserPrefab,
            new Vector3(
                _centerGameObject.transform.position.x + laserStartPos.x,
                _centerGameObject.transform.position.y + laserStartPos.y,
                0
            ),
            Quaternion.Euler(0, 0, laserAng)
        );
