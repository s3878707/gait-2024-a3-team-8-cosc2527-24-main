...
public class Spawner : MonoBehaviour
{
    ...
    [SerializeField]
    public GameObject _spawnerExplosion;
    ...
    IEnumerator SpawnEnemyRoutine()
    {
        ...
        {
            if (firstLoad)
            { // Delay the enemy spawning when first loaded
    ...
    public void Damaged()
    {
        ...
        if (hitpoints <= 0)
        {
            ...
            if (laserHits == 1)
            {
                ...
                _player.GetComponent<PlayerAgent>().HandleDestroySpawner();
    ...
    private void OnTriggerEnter2D(Collider2D other)
    {
        if (other.tag == "Laser")
        {
            ...
            _player.GetComponent<PlayerAgent>().HandleShootSpawners();
