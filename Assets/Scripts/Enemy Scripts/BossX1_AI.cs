...
public class BossX1_AI : MonoBehaviour
{
    ...
    [SerializeField]
    private GameObject _furyModeGlow;
    ...
    [SerializeField]
    public float movementRate; // How long it moves (Higher is faster)

    [SerializeField]
    public float movementPause; // How long it pauses (Lower is faster)
    ...
    private void FixedUpdate()
    {
        ...
        {
            player.GetComponent<PlayerAgent>().HandleCloseToBoss();
        }
        if (
            (
                transform.position.x >= (currentPlayerX - playerRadius)
                && transform.position.x <= (currentPlayerX + playerRadius)
            )
            && (
                transform.position.y >= (currentPlayerY - playerRadius)
                && transform.position.y <= (currentPlayerY + playerRadius)
            )
            && (isCharging == false)
            && (isCrossing == false)
        )
        {
            ...
            transform.position = new Vector3(
                currentPlayerX + RandomXoffset,
                currentPlayerY + RandomYoffset,
                0
            );
        }
        else
    ...
    private void OnTriggerEnter2D(Collider2D other)
    {
        if (other.tag == "Laser")
        {
            ...
            player.GetComponent<PlayerAgent>().HandleShootBoss();
        ...
        else if (other.tag == "Player")
        {
            Player player = other.GetComponent<Player>();
             other.GetComponent<PlayerAgent>().HandleHitBoss();
            if (player != null)
            {
    ...
    private void Damaged()
    {
        ...
        if (hitpoints <= 0)
        {
            player.GetComponent<PlayerAgent>().HandleDestroyBoss();
